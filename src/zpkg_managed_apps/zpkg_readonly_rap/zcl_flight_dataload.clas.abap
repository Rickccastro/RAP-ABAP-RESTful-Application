CLASS zcl_flight_dataload DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_flight_dataload IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.

     " Inserir Status
    TYPES tt_stat TYPE TABLE OF ztrap_fli_status WITH DEFAULT KEY.
    DATA(lt_status) = VALUE tt_stat(
        ( id = '1' description = 'Aguardando Aprovação' )
        ( id = '2' description = 'Aprovado' )
        ( id = '3' description = 'Reprovado' )
    ).
    MODIFY ztrap_fli_status FROM TABLE lt_status.

    " Inserir Cidades
    TYPES tt_city TYPE TABLE OF ztrap_city WITH DEFAULT KEY.
    ...
    MODIFY ztrap_city FROM TABLE lt_city.

    " Inserir Voos
    TYPES tt_flight TYPE TABLE OF ztrap_flight WITH DEFAULT KEY.
    ...
    MODIFY ztrap_flight FROM TABLE lt_flight.
